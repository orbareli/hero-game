...
# ------------------------------------------------------------------ #
def run_sql(query):
    print(f"DEBUG: Sending query -> {query[:50]}...") # Print first 50 chars
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        raw_res = b"".join(chunks).decode()
        if not raw_res:
            print("DEBUG: Received empty response from OrSQL!")
            return []
        
        response = json.loads(raw_res)
        if response.get("error"):
            print(f"DEBUG: SQL ERROR -> {response['error']}")
...
characters = [
    # ── HEROES ──────────────────────────────────────────────────────
    {
        ...
        "passive_desc": "Reduces all incoming damage by 8 perc",
        "skill_name":   "Shield Slam",
        "skill_desc":   "Deals 1 ATK damage and reduces enemy ATK by 15 perc for 2 turns",
        "skill_mult":   1,
        "skill_cd":     3,
    ...
    },
    {
        ...
        "passive_desc": "Gains plus perc CRIT chance when going first in a turn",
        "skill_name":   "Thunderstrike",
        "skill_desc":   "Deals 2ATK damage. Always crits if enemy HP is above 80 perc",
        "skill_mult":   2,
        "skill_cd":     4,
    ...
    },
    {
        ...
        "passive_desc": "Heals 5 perc of max HP at the start of each turn",
        "skill_name":   "Nova Burst",
        "skill_desc":   "Deals 2x ATK damage and heals self for 20 perc of damage dealt",
        "skill_mult":   2,
        "skill_cd":     3,
        "portrait_id":  "solaris",
    },
    # ── VILLAINS ────────────────────────────────────────────────────
    {
        ...
        "skill_desc":   "Deals 1x ATK and doubles poison damage this turn",
        "skill_mult":   1,
        "skill_cd":     3,
    ...
    },
    {
        ...
        "skill_desc":   "Deals 1x ATK damage. Ignores 50 perc of enemy DEF",
        "skill_mult":   1,
        "skill_cd":     4,
    ...
    },
    {
        ...
        "passive_desc": "Gains plus 10 perc ATK for every 20 perc HP lost",
        "skill_name":   "Void Rupture",
        "skill_desc":   "Deals 2x ATK damage. Gains 1 charge 3 charges triggers a free turn",
        "skill_mult":   2,
        "skill_cd":     5,
...
for c in characters:
    ...
    # Inside your insert loop in seed.py
    query = (
        f'INSERT INTO characters '
        f'(name, faction, rarity, base_hp, base_atk, base_def, base_spd, base_crit, base_evade, '
        f'passive_name, passive_desc, skill_name, skill_desc, skill_mult, skill_cd, portrait_id) '
        f'VALUES ("{c["name"]}", "{c["faction"]}", "{c["rarity"]}", ' # Using " instead of '
        f'{c["base_hp"]}, {c["base_atk"]}, {c["base_def"]}, {c["base_spd"]}, '
        f'{c["base_crit"]}, {c["base_evade"]}, '
        f'"{c["passive_name"]}", "{c["passive_desc"]}", '
        f'"{c["skill_name"]}", "{c["skill_desc"]}", '
